<?php

namespace app\database;

use Exception;
use app\exceptions\UserNameJustUsedException;
use app\database\interfaces\IUserDao;
use app\exceptions\UserJustSignUpException;
use app\exceptions\ImageException;
use app\exceptions\PasswordException;
use app\exceptions\SignUpTokenException;
use app\exceptions\UserConfirmException;
use app\exceptions\UserException;
use function utility\current_unixdatetime;
use function utility\formatDate;
use function utility\getToken;
use function utility\hashed_password;

class UserDao implements IUserDao
{

    private $daoFactory;

    public function __construct()
    {
        $this->daoFactory = DaoFactory::instance();
    }

    private static $instance;

    public static function instance()
    {
        if (self::$instance == null)
            self::$instance = new self();
        return self::$instance;
    }

    public function create($user)
    {
        $signup = $this->daoFactory->transaction(function () use ($user, &$token) {
            $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO user (name, last_name, gender, birth_date, email, phone, city, height) VALUES (?, ?, ?, ?, ?, ?, ?, ? )");
            if (!$stmt)
                throw new Exception("User : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $name = $user[NAME];
            $lastname = $user[LAST_NAME];
            $gender = $user[GENDER];
            $birth_date = formatDate($user[BIRTH_DATE]);
            $email = $user[EMAIL];
            $phone = $user[PHONE];
            $city = $user[CITY];
            $height = $user[HEIGHT];
            $stmt->bind_param("sssssssd", $name, $lastname, $gender, $birth_date, $email, $phone, $city, $height);
            if (!$stmt->execute()) {
                if ($this->daoFactory->isDuplicate())
                    throw new UserJustSignUpException();
                throw new Exception("User : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
            }
            $stmt->close();

            $id = $this->daoFactory->getConnection()->insert_id;

            $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO login(id_user, username, hash_password) VALUES (?, ?, ?)");
            if (!$stmt)
                throw new Exception("Login : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $username = $user[USERNAME];
            $hash = hashed_password($user[PASSWORD]);
            $stmt->bind_param("iss", $id, $username, $hash);
            if (!$stmt->execute()) {
                if ($this->daoFactory->isDuplicate())
                    throw new UserNameJustUsedException();
                throw new Exception("Login : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
            }
            $stmt->close();

            $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO target_default(id_user, target) VALUES (?, ?)");
            if (!$stmt)
                throw new Exception("Target : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $target = $user[TARGET];
            $stmt->bind_param("is", $id, $target);
            if (!$stmt->execute())
                throw new Exception("Target : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->close();

            $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO weight(id_user, value) VALUES (?,?)");
            if (!$stmt)
                throw new Exception("Weight : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $weight = $user[WEIGHT];
            $stmt->bind_param("id", $id, $weight);
            if (!$stmt->execute())
                throw new Exception("Weight : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->close();

            if (isset($user[IMG])) { // if image is set && not null
                $image = $user[IMG];
                $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO profile_image(id_user, name, content) VALUES (?, ?, ?)");
                if (!$stmt)
                    throw new Exception("Image : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("iss", $id, $image[IMG_NAME], $image[IMG_ENCODE]);
                if (!$stmt->execute())
                    throw new Exception("Image : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->close();
            }

            $token = getToken(10);

            $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO signup(id_user, token, date) VALUES (?, ?, ?)");
            if (!$stmt)
                throw new Exception("Token : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());

            $current_time = current_unixdatetime();
            $stmt->bind_param("iss", $id, $token, $current_time);
            if (!$stmt->execute())
                throw new Exception("Token : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->close();
        });
        return $signup ? $token : null;
//        if (
//            $this->daoFactory->transaction(function () use ($user, &$token, &$email) {
//                $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO user (name, last_name, gender, birth_date, email, phone, city, height) VALUES (?, ?, ?, ?, ?, ?, ?, ? )");
//                if (!$stmt)
//                    throw new Exception("User : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
//                $name = $user[NAME];
//                $lastname = $user[LAST_NAME];
//                $gender = $user[GENDER];
//                $birth_date = formatDate($user[BIRTH_DATE]);
//                $email = $user[EMAIL];
//                $phone = $user[PHONE];
//                $city = $user[CITY];
//                $height = $user[HEIGHT];
//                $stmt->bind_param("sssssssd", $name, $lastname, $gender, $birth_date, $email, $phone, $city, $height);
//                if (!$stmt->execute()) {
//                    if ($this->daoFactory->isDuplicate())
//                        throw new UserJustSignUpException();
//                    throw new Exception("User : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
//                }
//                $stmt->close();
//
//                $id = $this->daoFactory->getConnection()->insert_id;
//
//                $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO login(id_user, username, hash_password) VALUES (?, ?, ?)");
//                if (!$stmt)
//                    throw new Exception("Login : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
//                $username = $user[USERNAME];
//                $hash = hashed_password($user[PASSWORD]);
//                $stmt->bind_param("iss", $id, $username, $hash);
//                if (!$stmt->execute()) {
//                    if ($this->daoFactory->isDuplicate())
//                        throw new UserNameJustUsedException();
//                    throw new Exception("Login : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
//                }
//                $stmt->close();
//
//                $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO target_default(id_user, target) VALUES (?, ?)");
//                if (!$stmt)
//                    throw new Exception("Target : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
//                $target = $user[TARGET];
//                $stmt->bind_param("is", $id, $target);
//                if (!$stmt->execute())
//                    throw new Exception("Target : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
//                $stmt->close();
//
//                $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO weight(id_user, value) VALUES (?,?)");
//                if (!$stmt)
//                    throw new Exception("Weight : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
//                $weight = $user[WEIGHT];
//                $stmt->bind_param("id", $id, $weight);
//                if (!$stmt->execute())
//                    throw new Exception("Weight : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
//                $stmt->close();
//
//                if (isset($user[IMG])) { // if image is set && not null
//                    $image = $user[IMG];
//                    $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO profile_image(id_user, name, content) VALUES (?, ?, ?)");
//                    if (!$stmt)
//                        throw new Exception("Image : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
//                    $stmt->bind_param("iss", $id, $image[IMG_NAME], $image[IMG_ENCODE]);
//                    if (!$stmt->execute())
//                        throw new Exception("Image : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
//                    $stmt->close();
//                }
//
//                $token = getToken(10);
//
//                $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO signup(id_user, token, date) VALUES (?, ?, ?)");
//                if (!$stmt)
//                    throw new Exception("Token : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
//
//                $current_time = current_unixdatetime();
//                $stmt->bind_param("iss", $id, $token, $current_time);
//                if (!$stmt->execute())
//                    throw new Exception("Token : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
//                $stmt->close();
//            })
//        ) {
//            return Mailer::instance()->sendSignup($email, $user, $token);
//        }
//        return false;
    }

    public function checkSignUp($userCredentials)
    {
        $id_user = self::checkSignIn($userCredentials[USERNAME], $userCredentials[PASSWORD]);

        if (
            $this->daoFactory->selection(function () use ($userCredentials, $id_user) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT count(*) FROM session WHERE id_user=?");
                if (!$stmt)
                    throw new Exception("Check Token : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("i", $id_user);
                if (!$stmt->execute())
                    throw new Exception("Check Token: Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_result($justLogged);
                $stmt->fetch();
                $stmt->close();
                if ($justLogged > 0)
                    throw new UserConfirmException();

                $stmt = $this->daoFactory->getConnection()->prepare("SELECT count(*) FROM signup WHERE id_user=? and token=?");
                if (!$stmt)
                    throw new Exception("Check Token : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("is", $id_user, $userCredentials[TOKEN]);
                if (!$stmt->execute())
                    throw new Exception("Check Token: Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_result($isCorrect);
                $stmt->fetch();
                $stmt->close();

                if ($isCorrect == 0)
                    throw new SignUpTokenException();
            })
        ) {
            return $id_user;
        }

    }

    public function checkSignIn($username, $password)
    {

        if (
            $this->daoFactory->selection(function () use ($username, $password, &$id_user) {

                $stmt = $this->daoFactory->getConnection()->prepare("SELECT id_user, hash_password FROM login WHERE username=?");
                if (!$stmt)
                    throw new Exception("user id : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("s", $username);
                if (!$stmt->execute())
                    throw new Exception("Check : Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_result($id_user, $hash_password);
                $stmt->fetch();
                $stmt->close();

                if ($id_user == NULL && $hash_password == NULL)
                    throw new UserException();

                if (!password_verify($password, $hash_password))
                    throw new PasswordException();
            })
        ) {
            return $id_user;
        }

    }

    public function getUserForId($id)
    {
        $user = [];
        if (
            $this->daoFactory->selection(function () use ($id, &$user) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT u.name, u.last_name, u.gender, u.birth_date, u.email, u.phone, u.city, u.height, l.username
                                                                  FROM login l JOIN user u on(l.id_user=u.id_user) WHERE u.id_user=?");
                if (!$stmt)
                    throw new Exception("user from id : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("i", $id);
                if (!$stmt->execute())
                    throw new Exception("user from id : Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $result = $stmt->get_result();
                $user = $result->fetch_assoc();
                if (count($user) == 0)
                    throw new UserException();
                $stmt->close();
            })
        ) {
            return $user;
        }

    }

    public function getUserForUsername($username)
    {

        if (
            $this->daoFactory->selection(function () use ($username, &$id_user) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT id_user FROM login WHERE username=?");
                if (!$stmt)
                    throw new Exception("user id : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("s", $username);
                if (!$stmt->execute())
                    throw new Exception("Check : Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_result($id_user);
                $stmt->fetch();
                if ($id_user == NULL)
                    throw new UserException();
                $stmt->close();
            })
        ) {
            return [ID_USER => $id_user];
        }

    }

    public function allData($session_token, $_device)
    {

        if (
            $this->daoFactory->selection(function () use ($session_token, &$id_user, &$device) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT id_user, device FROM session WHERE token=? ");
                if (!$stmt)
                    throw new Exception("Check Token : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("s", $session_token);
                if (!$stmt->execute())
                    throw new Exception("Check Token: Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_result($id_user, $device);
                $stmt->fetch();
                $stmt->close();
                if ($id_user == NULL)
                    throw new UserException();
            })
        ) {
            $isDifferentDevice = $device == NULL || (strcmp($device, md5($_device)) != 0);
            if ($isDifferentDevice) {
                SessionDao::instance()->setNewDevice($_device, $id_user);
            }
            return [
                USER => self::getUserForId($id_user) + self::getImageProfileForIdUser($id_user, $isDifferentDevice),
                WEIGHTS => WeightDao::instance()->getAllForUser($id_user),
                WORKOUTS => WorkoutDao::instance()->getAllForUser($id_user),
                SETTINGS => SettingsDao::instance()->getAllForUser($id_user)
            ];
        }

    }

    private function getImageProfileForIdUser($id, $isDifferentDevice)
    {

        if (
            $this->daoFactory->selection(function () use ($id, $isDifferentDevice, &$image) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT name " . ($isDifferentDevice ? ", content " : "") . " FROM profile_image WHERE id_user=?");
                if (!$stmt)
                    throw new Exception("user image id : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("i", $id);
                if (!$stmt->execute())
                    throw new Exception("user image : Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $result = $stmt->get_result();
                $image = $result->fetch_assoc();
                $stmt->close();
            })
        ) {
            return [IMG => $image];
        }

    }

    public function getImageProfileForIdUserAndName($id, $name)
    {
        if (
            $this->daoFactory->selection(function () use ($id, $name, &$image) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT name, content FROM profile_image WHERE id_user=? and name = ?");
                if (!$stmt)
                    throw new Exception("user image id : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("is", $id, $name);
                if (!$stmt->execute())
                    throw new Exception("user image : Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $result = $stmt->get_result();
                $image = $result->fetch_assoc();
                $stmt->close();
                if ($image == NULL)
                    throw new ImageException();
            })
        ) {
            return [IMG => $image];
        }

    }

    private function getUserForEmail($email)
    {
        if (
            $this->daoFactory->selection(function () use ($email, &$id_user) {
                $stmt = $this->daoFactory->getConnection()->prepare("SELECT id_user FROM user WHERE email=?");
                if (!$stmt)
                    throw new Exception("user id : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param("s", $email);
                if (!$stmt->execute())
                    throw new Exception("Check : Selezione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_result($id_user);
                $stmt->fetch();
                $stmt->close();
            })
        ) {
            return [ID_USER => $id_user];
        }
    }

    public function requestForgotPassword($email, $c_key, $end_validity)
    {
        $user = self::getUserForEmail($email);
        $this->daoFactory->transaction(function () use ($user, $email, $c_key, $end_validity, &$requestAccepted) {
            $stmt = $this->daoFactory->getConnection()->prepare("SELECT count(*) FROM request_forgot_password WHERE email=?");
            if (!$stmt)
                throw new Exception("Check email : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->bind_param("s", $email);
            if (!$stmt->execute())
                throw new Exception("Check email : Selezione fallito. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->bind_result($row);
            $stmt->fetch();
            $stmt->close();
            if ($row == 0) {
                $sql = "INSERT INTO request_forgot_password (c_key, end_validity, email " . ($user[ID_USER] == null ? ") VALUES (?, ?, ?)" : ", id_user) VALUES (?, ?, ?, ?)");
            } else {
                $sql = "UPDATE request_forgot_password SET c_key=?, end_validity=? WHERE email = ?" . ($user[ID_USER] == null ? "" : " and id_user=?");
            }

            $stmt = $this->daoFactory->getConnection()->prepare($sql);
            if (!$stmt)
                throw new Exception("Forgot Password : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            if ($user[ID_USER] == null)
                $stmt->bind_param("sss", $c_key, $end_validity, $email);
            else
                $stmt->bind_param("sssi", $c_key, $end_validity, $email, $user[ID_USER]);
            if (!$stmt->execute())
                throw new Exception("Forgot Password : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());

            $requestAccepted = $stmt->affected_rows;
            $stmt->close();
        });
        return $requestAccepted;
    }


    public function update($user, $id_user)
    {
        $update = false;
        $this->daoFactory->transaction(function () use ($user, $id_user, &$update) {
            $keys = [];
            $values = [];

            $allowedKeys = [NAME, LAST_NAME, GENDER, BIRTH_DATE, EMAIL, CITY, PHONE, HEIGHT];

            foreach ($user as $key => $value) {
                if (in_array($key, $allowedKeys)) {
                    $keys[] = $key;
                    $values[] = $value;
                }
            }

            $values[] = $id_user;

            if (count($values) > 1) {
                $stmt = $this->daoFactory->getConnection()->prepare("UPDATE user SET " . join("=?,", $keys) . "=? WHERE id_user=?");
                if (!$stmt)
                    throw new Exception("User update : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                $stmt->bind_param(str_repeat('s', count($keys)) . "i", ...$values);
                if (!$stmt->execute())
                    throw new Exception("User : Update fallito. Errore: " . $this->daoFactory->getErrorConnection());
                $update = $update || $stmt->affected_rows;
                $stmt->close();
            }

            if (array_key_exists(IMG, $user)) {
                $image = $user[IMG];
                if ($image == null) {
                    // delete image
                    $stmt = $this->daoFactory->getConnection()->prepare("DELETE FROM profile_image WHERE id_user=?");
                    if (!$stmt)
                        throw new Exception("User Img update : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                    $stmt->bind_param("i", $id_user);
                    if (!$stmt->execute())
                        throw new Exception("User Img : Update fallito. Errore: " . $this->daoFactory->getErrorConnection());
                    $update = $update || $stmt->affected_rows;
                    $stmt->close();
                } else {
                    //update image
                    $stmt = $this->daoFactory->getConnection()->prepare("INSERT INTO profile_image (content, name, id_user) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE content=?, name=?;");
                    if (!$stmt)
                        throw new Exception("User Img update : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
                    $stmt->bind_param("ssiss", $image[IMG_ENCODE], $image[NAME], $id_user, $image[IMG_ENCODE], $image[NAME]);
                    if (!$stmt->execute())
                        throw new Exception("User Img : Update fallito. Errore: " . $this->daoFactory->getErrorConnection());
                    $update = $update || $stmt->affected_rows;
                    $stmt->close();
                }
            }
        });
        return $update;

    }

    public function changePassword($password, $id_user)
    {
        $changed = false;
        $this->daoFactory->transaction(function () use ($password, $id_user, &$changed) {
            $hash_crypt = hashed_password($password);
            $stmt = $this->daoFactory->getConnection()->prepare("UPDATE login SET hash_password=? WHERE id_user=?");
            if (!$stmt)
                throw new Exception("Password update : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->bind_param("si", $hash_crypt, $id_user);
            if (!$stmt->execute())
                throw new Exception("Password : Update fallito. Errore: " . $this->daoFactory->getErrorConnection());
            $changed = ($stmt->affected_rows == 1);
            $stmt->close();
        });
        return $changed;

    }

    public function delete($id_user)
    {
        $delete = false;
        $this->daoFactory->transaction(function () use ($id_user, &$delete) {
            $stmt = $this->daoFactory->getConnection()->prepare("DELETE FROM user WHERE id_user=?");
            if (!$stmt)
                throw new Exception("User delete : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
            $stmt->bind_param("i", $id_user);
            if (!$stmt->execute())
                throw new Exception("User : Delete fallito. Errore: " . $this->daoFactory->getErrorConnection());
            $delete = ($stmt->affected_rows == 1);
            $stmt->close();
        });
        return $delete;

    }
}


