...
use app\database\interfaces\ISettingsDao;
use Exception;
...
class SettingsDao implements ISettingsDao
{
  ...
  private function getUnitMeaureDefaultForUser($id_user)
  {
    ...
    if (
      $this->daoFactory->selection(function () use ($id_user, &$unit) {
        $stmt = $this->daoFactory->getConnection()->prepare("SELECT energy, distance, weight, height FROM unit_measure_default WHERE id_user=?;");
        if (!$stmt)
          throw new Exception("unit measure : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
        $stmt->bind_param("i", $id_user);
        if (!$stmt->execute())
          throw new Exception("unit measure : Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
  ...
  private function getDefault($type, $id_user)
  {
    ...
    if (
      $this->daoFactory->selection(function () use ($type, $id_user, &$def) {
        $stmt = $this->daoFactory->getConnection()->prepare("SELECT $type FROM " . $type . "_default WHERE id_user=?");
        if (!$stmt)
          throw new Exception("default $type : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
        $stmt->bind_param("i", $id_user);
        if (!$stmt->execute())
          throw new Exception("default $type: Inserimento fallito. Errore: " . $this->daoFactory->getErrorConnection());
        $result = $stmt->get_result();
  ...
  public function updateUnits($unit_measure, $id_user)
  {
    return $this->daoFactory->transaction(function () use ($unit_measure, $id_user) {
      ...
      $allowedKeys = [DISTANCE, WEIGHT, HEIGHT];

      foreach ($unit_measure as $key => $value) {
      ...
      $values[] = $id_user;
      $typeParam = str_repeat('s', count($keys)) . "i";
      if (count($values) > 1) {
        $stmt = $this->daoFactory->getConnection()->prepare("UPDATE unit_measure_default SET " . join("=?,", $keys) . "=? WHERE id_user=?");
        if (!$stmt)
          throw new Exception("Units update : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
        $stmt->bind_param($typeParam, ...$values);
        if (!$stmt->execute())
          throw new Exception("Units : Update fallito. Errore: " . $this->daoFactory->getErrorConnection());
        $stmt->close();
      }
    });
  }
  ...
  private function updateDeafult($type, $val, $id_user)
  {
    return $this->daoFactory->transaction(function () use ($type, $val, $id_user) {
      ...
      if (!$stmt)
        throw new Exception("$type update : Preparazione fallita. Errore: " . $this->daoFactory->getErrorConnection());
      $stmt->bind_param("si", $val, $id_user);
      if (!$stmt->execute())
        throw new Exception("$type : Update fallito. Errore: " . $this->daoFactory->getErrorConnection());
      $stmt->close();
    });
  }
