...
class UserDao implements IUserDao{
   ...
   public function create($user){
     ...
     if($this->daoFactory->transaction(function() use ($user, &$token, &$id, &$email){
   ...
   public function getImageProfileForIdUserAndName($id, $name){
    ...
    private function getUserForEmail($email){
     if($this->daoFactory->selection(function() use ($email, &$id_user){
   ...
   public function requestForgotPassword($email, $c_key, $end_validity){
     ...
     return $this->daoFactory->transaction(function() use ($user, $email, $c_key, $end_validity){
                 ...
                 $stmt->close();
                 if($row==0){
                    $sql = "INSERT INTO request_forgot_password (c_key, end_validity, email " .
                           ( $user[ID_USER]==null ? ") VALUES (?, ?, ?)" : ", id_user) VALUES (?, ?, ?, ?)");
                 }else{
                    $sql = "UPDATE request_forgot_password SET c_key=?, end_validity=? WHERE email = ?" .
                              ( $user[ID_USER]==null? "" : " and id_user=?");
                 }
